...
from hydragnn.utils.model import CompetitionGatedLoss
...
def train_validate_test_multifidelity(
    model,
    optimizer,
    train_loader,
    val_loader,
    ...
    for epoch in range(0, num_epoch):
        ...
        with profiler as prof:
            train_loss, train_taskserr = train_multifidelity(
                train_loader, model, optimizer, verbosity, profiler=prof
            )
        val_loss, val_taskserr = validate_multifidelity(val_loader, model, verbosity)
        test_loss, test_taskserr, true_values, predicted_values = test_multifidelity(
            test_loader,
            model,
            verbosity,
            reduce_ranks=True,
