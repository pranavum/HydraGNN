...
if __name__ == "__main__":
    ...
    os.environ["SERIALIZED_DATA_PATH"] = dirpwd + "/gan_dataset/"+"GAN"
    config["Dataset"]["path"]["total"] = dirpwd + "/gan_dataset/"+"GAN"
    ...
    if not args.loadexistingsplit:
        ...
        if args.format == "pickle":
            basedir = os.path.join(
                os.path.dirname(__file__), "gan_dataset", "serialized_dataset"
            )
            SerializedWriter(
                total,
                basedir,
    ...
    with open("train_dataset/serialized_dataset/NiNb-trainset.pkl", "rb") as f:
        _ = pickle.load(f)
        training_minmax_graph_feature = pickle.load(f)

    test_minmax_graph_feature = []

    with open("gan_dataset/serialized_dataset/NiNb_0-testset.pkl", "rb") as f:
        _ = pickle.load(f)
