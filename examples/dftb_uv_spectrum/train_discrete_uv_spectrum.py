...
if __name__ == "__main__":
    ...
    datafile = os.path.join(dirpwd, "dataset/dftb_gdb9_smooth_spectrum")
    ##################################################################################################################
    ...
    if args.mae:
            ...
            for head_idx in range(len(graph_feature_dim)):
                ...
                for sample_id in range(0,num_test_samples):
                    true_sample = head_true[
                                  (sample_id * graph_feature_dim[ihead]):(sample_id + 1) * graph_feature_dim[ihead]]
        ...
        for sample_id in range(0, num_test_samples):
            ...
            pred_location_peaks = predicted_values[0][(sample_id * graph_feature_dim[0]):(sample_id + 1) * graph_feature_dim[0]]
            pred_intensity_peaks = predicted_values[1][(sample_id * graph_feature_dim[0]):(sample_id + 1) * graph_feature_dim[0]]

            true_location_peaks = true_values[0][(sample_id * graph_feature_dim[0]):(sample_id + 1) * graph_feature_dim[0]]
            true_intensity_peaks = true_values[1][(sample_id * graph_feature_dim[0]):(sample_id + 1) * graph_feature_dim[0]]

            fig, ax = plt.subplots()
            ax.vlines(x=true_location_peaks, ymin=[0]*len(pred_location_peaks), ymax=true_intensity_peaks, colors='blue', ls='-', lw=4,
                       label='TD-DFTB+')
            ax.vlines(x=pred_location_peaks, ymin=[0]*len(pred_location_peaks), ymax=pred_intensity_peaks, colors='orange', ls='--', lw=4,
                       label='HydraGNN')
            plt.draw()
            plt.tight_layout()
